...
module Exercises = struct
  ...
  (* let opponents_winning_moves = winning_moves ~me:(Game.Piece.flip me)
     game in let all_possible_moves = available_moves game in List.filter
     all_possible_moves ~f:(fun pos -> List.exists opponents_winning_moves
     ~f:(fun bad_space -> Game.Position.equal pos bad_space)) *)
